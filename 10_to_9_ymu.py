...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        uic.loadUi(resource_path('10_to_9_ymu.ui'), self)
...
class Port:
    ...
    def count_ftr_ptr(src_path_list) -> dict:
        ...
        for count, src_path in enumerate(src_path_list):
            ...
            with open(src_path, "rb") as SOURCE:
                ...
                hdr_ptr_int = int.from_bytes(hdr_ptr, byteorder="big")
                ...
                ftr_ID_first = int.from_bytes(SOURCE.read(4), byteorder="big")
                if ftr_ID_first == 5:
                    ftr_ptr_1 = SOURCE.read(4)
                    ftr_data_1_start = int.from_bytes(ftr_ptr_1, byteorder="big")
                else:  # ftr_ID_first == 0
                    ftr_ptr_2 = SOURCE.read(4)
                    ftr_data_2_start = int.from_bytes(ftr_ptr_2, byteorder="big")

                '''Determine # of footer data sections'''
                SOURCE.seek(total_size - 4)
                last_4_bytes = SOURCE.read(4)
                last_4_bytes_int = int.from_bytes(last_4_bytes, byteorder="big")
    ...
    def read_hex(src_path: Path):
        ...
        with open(src_path, "rb") as SOURCE:
            ...
            '''Table info & data'''
            table_size = int.from_bytes(table_count_src, byteorder="big") * 16
            table_start = int.from_bytes(table_addr, byteorder="big")
            ...
            '''
            meta_start = int.from_bytes(meta_data_addr)
            frame_start = int.from_bytes(frame_data_addr)
            ...
            if hdr_ptr_int == 0:        # if no footer data exists, ftr_ID = 0
                ...
                ftr_start = int.from_bytes(ftr_ptr, byteorder="big")
                ...
                ftr_3_start = int.from_bytes(ftr_3_ptr, byteorder="big")
                if ftr_3_start != 0:
                    ftr_ID = 3
                elif int.from_bytes(ftr_ID_data) == 0:
                    ftr_ID = 2
                else:
                    ftr_ID = 1
            ...
            if ftr_ID == 3:
                ...
                ftr_1_start = int.from_bytes(ftr_1_ptr, byteorder="big")
    ...
    def wp_vis_edit(ftr_ID: int, dest_path: Path, data_dict: dict):
        ...
        table_count_dest_int = int.from_bytes(table_count_src) + hidden_wp_count
        ...
        table_count_diff = table_count_dest_int - int.from_bytes(table_count_src)
        ...
        frame_data_addr_int = int.from_bytes(
            frame_data_addr) + table_size_diff + meta_data_size_diff
        ...
        frame_entry_count = int.from_bytes(frame_entry_count)
        ...
        table_count_src_int = int.from_bytes(table_count_src)
        table_rows = []
    ...
    def count_skeleton_wp(input_pack):
        with (open(input_pack.joinpath('skeleton.g'), 'rb') as skeleton):
            ...
            str_pool_ptr = int.from_bytes(str_pool_ptr)
...
def load_anim_data_fe10():
    anim_data_file = resource_path('FE10_Anim_Data.txt')
